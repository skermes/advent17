...
(def puzzle-input "pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
(defn weakly-valid-passphrase? [passphrase]
...
(defn strongly-valid-passphrase? [passphrase]
  (let [words (s/split passphrase #"\s+")]
    (not (any? (map #(apply anagram? %) (combo/combinations words 2))))))

(defn valid-passphrases [condition text]
  (->> text
       s/split-lines
       (filter condition)))

(defn part-one [] (count (valid-passphrases weakly-valid-passphrase? puzzle-input)))

(defn part-two [] (count (valid-passphrases strongly-valid-passphrase? puzzle-input)))
