...
(def puzzle-input "116  1470  2610  179 2161  2690  831 1824  2361  1050  2201  118 145 2275  2625  2333
...
(defn parse-line [line]
  (map #(Integer/parseInt %) (s/split line #"\s+")))

(defn spread [parsed-line]
  (- (apply max parsed-line) (apply min parsed-line)))

(defn cartesian-product [as bs]
  (mapcat (fn [a] (map (fn [b] [a b]) bs)) as))
...
(defn only-divisors [parsed-line]
  (let [pairs (cartesian-product parsed-line parsed-line)
        dividable (firstp divisors? pairs)
        [a b] dividable]
    (/ a b)))

(defn checksum [data line-fn]
  (->> data
       s/split-lines
       (map parse-line)
       (map line-fn)
...
(defn part-one [] (checksum puzzle-input spread))

(defn part-two [] (checksum puzzle-input only-divisors))
